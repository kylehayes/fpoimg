...
class TestLayoutText:
    ...
    def test_single_text_is_centered_horizontally(self):
        ...
        _, _, (x, _) = result[0]
        # x should be roughly centered (not at 0 or at edge)
        assert 0 < x < 400
    ...
    def test_text_scales_down_when_too_wide(self):
        # Very narrow canvas should trigger reduction
        ...
        # Narrow canvas should use a smaller font
    ...
    def test_text_scales_down_when_too_tall(self):
        # Very short canvas with multiple lines
        texts = [
        ...
        # All texts should still be present
        assert len(result) == 3
        # Fonts should be reduced
    ...
    def test_minimum_font_size_is_one(self):
        # Extremely tiny canvas — font should never go below 1
